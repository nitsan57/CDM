...
class Experiment:
    ...
    def __init__(self, name, exp_id, adv_env_name='MultiGrid-Adversarial-v0',
        ...
        # ['SolvedPathLength', 'adversary_env_AdversaryReward']
        self.metrics_df = pd.DataFrame(pd.DataFrame({'SolvedPathLength': [0], 'adversary_env_AdversaryReward': [0], 'run': 'eval', 'step': 165000}, index=['p']))
        self.metrics = ['SolvedPathLength', 'adversary_env_AdversaryReward']
    ...
    def initialize_video_for_seed(self, seed, env_name, agent_id=None,
        ...
        vid_recorder = None  # mp4_video_recorder.Mp4VideoRecorder(vid_path, self.fps)
    ...
    def create_env_with_adversary(self, policy, recorder):
        ...
        if self.verbose:
            print('\tAdversary is building the environment...')
...
def generate_results(root_dir, experiments, test_on_test=False, test_mini=False,
    ...
    hparam_csv = r"C:/Users/Nitsan/Documents/Study/Thesis/ai/test.csv"
    with tf.io.gfile.GFile(hparam_csv, 'rb') as f:
...
def main(_):
    ...
    run_exp([Experiment('paired', 17485886, root_dir=FLAGS.root_dir, num_agents=3,
                        benchmark_against=[17486166, 17486367])])
