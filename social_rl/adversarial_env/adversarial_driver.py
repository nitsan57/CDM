...
from social_rl.adversarial_env.curriculum_env_rating import choose_best_env
...
class AdversarialDriver(object):
    ...
    def run(self, random_episodes=False):
        ...
        if random_episodes:
            ...
            if self.combined_population:
                ...
                agent_r_max, train_idxs = self.adversarial_episode()  # self.adversarial_episode_heuristic()  # self.adversarial_episode()
        else:
    ...
    def adversarial_episode_heuristic(self):
        ...
        orig_env_list = [AdversarialTFPyEnvironment(orig_data) for i in range(20)]
        filled_base_env_list = []
        for i in range(len(orig_env_list)):
            _, _, env_idx = self.run_agent(
                orig_env_list[i], self.adversary_env, self.env.reset, self.env.step_adversary)
            filled_base_env_list.append(orig_env_list[i]._envs[-1])  # FUNCTION NEED THE BASE ENV FOR SAMPLE
        # choose_best_env
        ...
        # agent = self.agent[0]  # get first agent #TODO SUPPORT MULTI AGENT
        # policy = agent.eval_policy
        # # observers = agent.eval_metrics
        # policy_state = policy.get_initial_state(orig_env_list[0].batch_size)
        # action_step = policy.action(time_step, policy_state)
        # policy = agent.eval_policy

        # idx = choose_best_env(filled_base_env_list)
...
#        agent_r_avg, agent_r_max, agent_idx = self.run_agent(
    ...
    def run_agent(self, env, agent_list, reset_func, step_func, agent_idx=None):
        ...
        print("AGNET_RUNNING:", agent.name)
        ...
        while num_steps < agent.max_steps:
            ...
            if False:  # for debug only
                ...
                print("STEP NUMBER", num_steps.numpy())
